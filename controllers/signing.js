...
/** Functions to be exported from module */
...
const generateKeyPem = async () => {
  /**
   * Default options for generating a key
   */
  const options = {
    name: "RSASSA-PKCS1-v1_5",
    // Consider using a 4096-bit key for systems that require long-term security
...
const signMessageB64 = async (privateKey, message) => {
  const signature = await signMessage(privateKey, message);
  const signatureB64 = btoa(signature.toString());
  return signatureB64;
};

const verifyMessageB64 = async (signatureB64, message) => {
